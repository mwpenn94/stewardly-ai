/**
 * ═══════════════════════════════════════════════════════════════════════════
 * ATLAS Memory Store — MemoryStore Implementation
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Connects the platform-agnostic memory engine to ATLAS's Drizzle ORM
 * schema. Implements the MemoryStore interface from @platform/intelligence.
 *
 * TiDB Coercion (Training System P-02):
 *   All numeric fields (confidence, etc.) are coerced at the DB boundary
 *   via coerceNumeric from ./dbCoercion.ts.
 */

import type { MemoryStore, StoredMemory, StoredEpisode, EpisodeSummary } from "./memoryEngine";
import { coerceNumeric } from "./dbCoercion";
import { getDb } from "../../db";
import { memories, memoryEpisodes } from "../../../drizzle/schema";
import { eq, desc } from "drizzle-orm";

export const atlasMemoryStore: MemoryStore = {
  async getMemories(userId: number, limit = 30): Promise<StoredMemory[]> {
    ...
    return rows.map((r) => ({
      ...
      // P-02: coerce confidence at DB boundary (TiDB returns decimal as string)
      confidence: coerceNumeric(r.confidence, 0.5),
...
export default atlasMemoryStore;
