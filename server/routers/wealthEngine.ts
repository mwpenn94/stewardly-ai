...
const HolisticConfigSchema = z
  .object({
    ...
    investmentReturn: z.number().min(-0.5).max(0.5).optional(),
    inflationRate: z.number().min(-0.1).max(0.3).optional(),
  ...
  sensitivitySweep: protectedProcedure
    .input(
      z.object({
        ...
        xRange: z.tuple([z.number().min(-1e6).max(1e6), z.number().min(-1e6).max(1e6)]),
        yRange: z.tuple([z.number().min(-1e6).max(1e6), z.number().min(-1e6).max(1e6)]),
