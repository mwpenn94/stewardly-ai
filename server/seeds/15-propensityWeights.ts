...
export async function seed() {
  ...
  console.info(`[seed:15] Propensity weights: ${EXPERT_MODELS.length} segment models loaded`);
  EXPERT_MODELS.forEach(m => console.info(`  - ${m.segment}: ${Object.keys(m.features).length} features`));
}
