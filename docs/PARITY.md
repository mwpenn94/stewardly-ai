...
| PARITY-CALC-0002      | Calculator Monte Carlo fan chart on projections | wealth_engine  | open         | P2       | build      | 0     |             | MonteCarloFan.tsx component exists but isn't wired into Retirement or StrategyComparison pages. Backend `uweMonteCarlo` procedure available. |
| PARITY-CALC-0003      | Income stream breakdown visualization    | wealth_engine        | open         | P2       | build      | 0     |             | IncomeStreamBreakdown.tsx component exists but isn't wired into PracticeToWealth or TeamBuilder. BIE engine returns per-stream data. |
