...
| PARITY-CALC-0001      | Rich reference integration on wealth-engine pages | wealth_engine   | in_progress  | P1       | build      | 3     | (BL P2)     | BL Pass 1-2: StrategyComparison enriched with stress test (3 scenarios), historical backtest, industry benchmarks (proper data rendering), year-by-year detail, guardrail warnings, product references (14 products). Retirement enriched with GFC stress test, backtest, benchmarks. Still need: Monte Carlo bands on chart. |
...
| PARITY-CALC-0003      | Product reference citations inline on wealth pages | wealth_engine  | done         | P2       | build      | 2     | (BL P2)     | BL Pass 2: StrategyComparison now shows collapsible product references panel with 14 product citations + benchmarks inline. Retirement still needs product refs. |
| PARITY-CALC-0004      | Guardrail validation feedback on all calculator inputs | wealth_engine | in_progress | P2    | build      | 2     | (BL P3)     | BL Pass 3: Reusable `GuardrailWarning.tsx` component with client-side guardrail checks. WhatIfSensitivity got contextual guardrail note with industry references. QuickQuoteFlow got score-based product reference insights (shows weak areas + citations). TeamBuilder still needs guardrail integration. |
