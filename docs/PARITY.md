...
- Pass 12 · angle: type safety follow-up · queue: [F1 remove 4 `as any` casts in githubClient.ts oauth2 load path + normalize expires_at across string/seconds/ms] · items completed: [new normalizeExpiresAt helper in githubClient.ts that accepts ISO string, epoch seconds (< 1e12 heuristic), or epoch ms (>= 1e12) and emits a single ISO string, rejects invalid dates + NaN/Infinity/arrays/objects; rewrote oauth2 credential decoding block to drop `(raw as any).access_token/refresh_token/expires_at/scope` in favor of Record<string, unknown> narrowing with per-field typeof gating; 8 new tests covering ISO pass-through + ISO round-trip normalization + invalid string reject + epoch seconds + epoch ms + non-finite number reject + array/object reject + null/undefined] · items deferred: [] · sha: pending-commit
