...
| PARITY-PERF-0003      | Duplicate voicesQuery in Settings + VoiceTab          | performance    | open         | P3       | build      | 0     |             | Same trpc.voice.voices.useQuery() called in parent Settings and child VoiceTab. Low priority since staleTime now prevents double-fetch. |
...
| PARITY-SEC-0007       | OAuth state/nonce not validated | security | open | P2 | build | 0 | | CBL17 audit finding: OAuth callbacks (/api/auth/google/callback, /api/auth/linkedin/callback) parse state from base64 but never validate it against the original request. Nonce is generated but never verified against ID token. Missing CSRF protection. |
| PARITY-SEC-0008       | Professional data exposed via publicProcedure | security | open | P2 | build | 0 | | CBL17 audit finding: professionals.list and professionals.match are publicProcedure — expose email, phone, website, firm, credentials to unauthenticated users. Could be used for targeted social engineering. Consider protectedProcedure or data filtering. |
| PARITY-SEC-0009       | Open redirect risk in OAuth initiateLinkedIn/Google | security | open | P2 | build | 0 | | CBL17 audit finding: authEnrichment.initiateLinkedIn and initiateGoogle accept user-provided redirectUri (Zod validates .url() but no whitelist). Could enable open redirect attacks. Need redirect URI whitelist. |
