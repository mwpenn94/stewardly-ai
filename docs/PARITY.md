...
| G16 | perf        | Virtualize message list when >100 messages.                                                        | Cursor, VS Code                  | P2       | done (pass 18) | 3     | build  | Shipped pass 18 (windowing core). `messageVirtualization.ts` exposes `computeWindow(messageIds, opts)` that activates above 100 messages and returns the union of head/anchor/tail/pinned visible indices, deduped and sorted. `segmentWindow(win)` collapses consecutive visible indices into runs interleaved with `hidden` markers (one placeholder per gap, not per skipped message). Avoids the react-window dep + variable-height layout-thrash. 19 new tests. **Renderer integration deferred to a follow-on pass.** |
...
Pass 18 · angle: performance · queue: G16 (message list virtualization core) · shipped: messageVirtualization.ts (computeWindow head/anchor/tail/pinned union + segmentWindow run-collapse + countRendered) + 19 new tests · deferred: G5, G6, G8, G9, G11, G12-UI, G16-UI, G18, G22, G24
