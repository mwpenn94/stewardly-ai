...
Pass 15 · angle: consumer coverage — PIL feedback dispatch mass wire-up · queue: G1 (feedback layer depth), G8 (PIL consumed anywhere), G21 (haptic triggered), uxPolish.test.ts regression · commit SHA: TBD · shipped: `sendFeedback()` fire-and-forget helper in feedbackSpecs.ts (avoids context dependency for all consumers); 10 new dispatch sites across 10 files: LearningQuizRunner (answer correct/incorrect), LearningFlashcardStudy (flip + answer), LearningDueReview (flashcard + question answers), ExamSimulator (exam_complete with percentage + weak topics), CaseStudySimulator (case_complete with percentage + compliance flags), OnboardingFlow (step_complete + complete), AudioPreferences (speed_changed + preferences_saved), MyFinancialTwin (visibility_changed), CodeChat/GitHubPanel (codechat.connected via useEffect), Retirement (calculator.result on mutation success). Fixed stale uxPolish.test.ts (was checking AppShell for prefetchRoute removed in Pass 9 — now checks Chat.tsx). Total active dispatch sites: 16 files, 22 unique feedback keys. G1 depth 4→7, G8 depth 0→7. Tests: 4,596 passing (173 files), +1 from uxPolish fix. TS clean, 0 regressions · deferred: remaining consumers (ComplianceAudit needs server-side events, AchievementSystem uses mock data), G21 haptic callers outside FeedbackDispatcher
...
| PARITY-CALC-0001      | Rich reference integration on wealth-engine pages | wealth_engine   | in_progress  | P1       | build      | 2     | (BL P1)     | BL Pass 1: StrategyComparison + Retirement pages enriched with stress test (3 scenarios), historical backtest (survival rate + best/worst/median), industry benchmarks context strip, year-by-year detail table, guardrail warnings. Still need: Monte Carlo bands on chart, product reference panel inline. |
| PARITY-CALC-0002      | Monte Carlo uncertainty bands on projection charts | wealth_engine  | open         | P2       | build      | 0     |             | Monte Carlo simulation exists in the engine (monteCarloSimulate) but percentile bands are not rendered on the Retirement or StrategyComparison projection charts. Need to overlay 10th/25th/50th/75th/90th bands. |
| PARITY-CALC-0003      | Product reference citations inline on wealth pages | wealth_engine  | open         | P2       | build      | 0     |             | 14 product references with citations exist in benchmarks.ts + SCUI. Only surfaced on EngineDashboard via ProductReferencePanel. StrategyComparison and Retirement pages need inline product reference access. |
| PARITY-CALC-0004      | Guardrail validation feedback on all calculator inputs | wealth_engine | open      | P2       | build      | 0     |             | checkGuardrail() exists server-side but warnings are not shown during input on most calculator pages. Only StrategyComparison has a basic guardrail warning strip. WhatIfSensitivity, TeamBuilder, QuickQuote need the same. |
