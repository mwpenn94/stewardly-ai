...
| G5  | Wire profile store into Wealth-Engine pages (Retirement, StrategyComparison, PracticeToWealth) | engines | P1 | in_progress | build | pass 9 (Retirement) | 0.25 | Retirement.tsx now reads useFinancialProfile, seeds all 4 inputs from the profile, applies late hydration via useRef guard, drops a banner + CompletenessGate around the Goal run button, persists on run. StrategyComparison + PracticeToWealth pending. |
