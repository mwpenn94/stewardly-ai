...
| 16   | planning-page-activation | Wire 5 stub planning pages to real calculator backends (PARITY-PLAN-0001) | (pending) | TaxPlanning.tsx → taxProjector.project/multiYear/rothComparison (3 tRPC endpoints), EstatePlanning.tsx → client-side estate tax math (current law + TCJA sunset + growth projection + document checklist), RiskAssessment.tsx → 7-question weighted risk questionnaire + 5-tier allocation profiles, IncomeProjection.tsx → configurable income sources + ssOptimizer.optimize + Monte Carlo sustainability + year-by-year projection, InsuranceAnalysis.tsx → DIME-method needs calculator + policy tracker + gap analysis. 0 regressions (4596/4596 passing, 18 env-dependent unchanged). | — |
