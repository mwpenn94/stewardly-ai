...
| PARITY-CATALOG-0002   | Non-US comparables                       | n/a                  | open         | P3       | build      | 0     |             | Found during Pass 9 build. Catalog is entirely US-centric. Need SJP (UK), Schroders, AJ Bell, Nomura Wealth (APAC) to serve an international advisor base.          |
| PARITY-TAX-0003       | State tax tables (NY/CA/IL/TX)           | tax_planning         | open         | P2       | build      | 0     |             | Found during Pass 4 build. The projector models federal only; state tax materially changes effective rate especially for CA/NY.                                    |
...
| PARITY-PORT-0003      | Short-position ledger tracking           | portfolio_mgmt       | open         | P3       | build      | 1     |             | Found during Pass 5 build. The ledger emits SHORT_POSITION warnings but doesn't actually open a short lot for the caller to track. Needed for options / hedging strategies. |
| PARITY-API-0002       | Webhook delivery (outbound events)       | api_first            | open         | P3       | build      | 1     |             | Found during Pass 6 build. Public /api/v1 is read-only. Advisors want webhook notifications when a client uploads a doc, completes a meeting, or crosses a drift threshold. |
