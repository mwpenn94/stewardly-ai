...
| G13 | client      | Voice input for the prompt bar — browser SpeechRecognition.                                        | Cursor Composer voice            | P2       | done (pass 16) | 3     | build  | Shipped pass 16 (adapter). `voiceInput.ts` exports `isVoiceInputSupported()` (SSR-safe feature detection of standard + webkit prefix), `createVoiceRecognizer({lang, continuous, interimResults, onTranscript, onFinal, onError, onEnd, onStateChange})` (wraps native SpeechRecognition into a typed adapter with start/stop/abort + state machine), `spliceTranscript(input, cursor, chunk)` (smart-spaced merge for partial dictation). 14 new tests with a node-env window/SpeechRecognition shim. **Mic button + popover wiring deferred to a follow-on pass.** |
