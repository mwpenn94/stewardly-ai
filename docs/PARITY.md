...
| G36 | role="tablist" / tabpanel on custom tab UIs | ✓ 6 bespoke tab UIs fixed (Build Loop CBL15) | 9/10 | P2 | S | Yes | Build Loop | done |
...
| PARITY-ERR-0004       | Chat.tsx consensus/visual mutations lack onError     | error_states   | open         | P2       | build      | 0     |             | consensusQuery, consensusStreamMutation, visualMutation still fire without onError. Lower priority since these are advanced features. |
| PARITY-ERR-0005       | No route-level ErrorBoundary on wealth-engine pages  | error_states   | done         | P2       | build      | 3     | (CBL20)     | 6 wealth-engine pages now wrapped with SectionErrorBoundary inside AppShell: Retirement, StrategyComparison, TeamBuilder, QuickQuote, BusinessIncome, WhatIfSensitivity. Calculator crash now shows recovery card instead of white screen. |
...
| PARITY-TEST-0001      | statisticalModels.ts 8 financial functions zero tests  | test_coverage  | done         | P0       | build      | 3     | (CBL16)     | 28 new tests: optimizeDebt (12), scoreRiskTolerance (10), monteCarloRetirement (6). Found + fixed allocation normalization bug (allocation sum > 100%). |
| PARITY-BUG-0001       | scoreRiskTolerance allocation could sum to >100%       | correctness    | done         | P0       | build      | 3     | (CBL16)     | stocks+bonds+alternatives computed independently via Math.round could exceed 100 before cash = max(0, 100-rest). Added normalization scale-down step. |
| PARITY-NAV-0004       | 23 orphaned routes with no discovery path              | navigation     | in_progress  | P1       | build      | 1     | (CBL17)     | 23 fully built pages with zero nav entries. CBL17 surfaced 6 highest-impact: Business Income, Social Security, Medicare, Products, Changelog + removed duplicate Calculators and Documents entries. 17 orphaned routes remain (mostly admin/redirect). |
| PARITY-NAV-0005       | Duplicate Calculators + Documents nav entries           | navigation     | done         | P2       | build      | 3     | (CBL17)     | Two identical "Calculators" entries and two "Documents" entries in TOOLS_NAV. Removed duplicates. |
| PARITY-SEC-0001       | Tax router unbounded financial fields                  | input_valid    | done         | P0       | build      | 3     | (CBL18)     | 6 USD fields in yearCtxSchema had no min/max — users could submit negative income or $100B+ amounts. Added .min(0).max(100_000_000) to all. |
| PARITY-SEC-0002       | Learning definition field unbounded string              | input_valid    | done         | P1       | build      | 3     | (CBL18)     | createDefinition accepted unlimited-length definition string (only had .min(1)). Added .max(50_000) — more than enough for any real definition. |
| PARITY-SEC-0003       | WealthEngine rate/return fields unbounded               | input_valid    | done         | P1       | build      | 3     | (CBL18)     | investmentReturn + inflationRate had no bounds — user could send 500% return. Added .min(-0.5).max(0.5) for return, .min(-0.1).max(0.3) for inflation. Sensitivity sweep ranges bounded to ±1M. |
| PARITY-SEC-0004       | WealthEngine .passthrough() allows arbitrary fields     | input_valid    | open         | P2       | build      | 0     |             | HolisticConfigSchema uses .passthrough() (line 178) — bypasses schema validation on unknown fields. Should be .strict() or removed. |
...
| CBL20 | graceful degradation | [R1 from PARITY-ERR-0005: wealth-engine pages lack ErrorBoundary] | (pending) | Wrapped 6 wealth-engine pages with SectionErrorBoundary inside AppShell: Retirement, StrategyComparison, TeamBuilder, QuickQuote, BusinessIncome, WhatIfSensitivity. Calculator rendering errors now show a recovery card with retry button instead of a white screen crash. ERR-0005 closed. Build: 21s. Tests: 0 regressions. | — |
| CBL19 | docs staleness | [A1: CLAUDE.md test counts stale (3,103→4,958), A2: nav item counts stale (42→67)] | (pending) | Updated CLAUDE.md test metrics (4,958 passing / 186 files / 204 total / 18 env-failing / 5,080 total tests) and navigation counts (67 items across 3 arrays). Build: 22s. | — |
| CBL18 | input validation + security | [A1: tax router unbounded USD fields, A2: learning unbounded string, A3: wealthEngine unbounded rates + sweep ranges] | (pending) | Tax: 6 USD fields gained .min(0).max(100M). Learning: definition field gained .max(50K). WealthEngine: investmentReturn bounded [-0.5, 0.5], inflationRate [-0.1, 0.3], sensitivity sweep ranges ±1M. Build: 21s. Tests: 4958 passing, 0 regressions. | PARITY-SEC-0004 (.passthrough() cleanup) deferred. |
| CBL17 | cross-app cohesion + nav | [A1: 23 orphaned routes, A2: duplicate nav entries, A3: surface 6 high-impact pages] | (pending) | Found 23 fully built pages with zero discovery paths. Surfaced 6: Business Income, Social Security, Medicare, Products in Intelligence section; Changelog in Utility nav. Removed duplicate Calculators and Documents entries. CommandPalette auto-derives from nav. Build: 22s. Tests: 10 CommandPalette tests pass. | 17 orphaned routes remain (admin/redirect/lower priority). |
| CBL16 | test coverage + correctness | [A1: statisticalModels.ts zero tests, A2: allocation normalization bug] | (pending) | 28 new tests for statisticalModels.ts covering optimizeDebt (12 tests: strategy comparison, payoff ordering, interest savings, invariants), scoreRiskTolerance (10 tests: score bounds, category assignment, allocation normalization, dimension scores, behavioral insights), monteCarloRetirement (6 tests: structural output, percentile ordering, year-by-year shape, contribution bounds). Found + fixed allocation normalization bug (stocks+bonds+alts could exceed 100% before cash computation). Tests: 4958 passing (+28), 186 files. Build: 22s. | — |
| CBL15 | accessibility + responsive | [R1 from PARITY G36: bespoke tab UIs missing ARIA roles] | (pending) | Added role="tablist"/role="tab"/aria-selected to 6 custom tab implementations: DisciplineDeepDive (+ tabpanel), AISettings, AITuningTab, SettingsHub, PlatformGuide, Marketplace. G36 closed. Build: 22s. Tests: 4930 passing, 0 regressions. | — |
