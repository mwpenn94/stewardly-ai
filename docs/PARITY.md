...
| G18 | i18n / currency-locale preferences: app hardcodes en-US + USD | i18n | P2 | done | build | pass 13 | 0.55 | New `client/src/stores/localePreferences.ts` pure helper with LocalePreferences shape + 13 locales + 9 currencies + formatCurrency/formatNumber/formatPercent with compact threshold + Intl fallback + 24 unit tests. Next pass can wire this into the React tree and swap `@/lib/wealth-engine/animations.formatCurrency` call sites. |
| G5  | Wire profile store into Wealth-Engine pages (Retirement, StrategyComparison, PracticeToWealth) | engines | P1 | done | build | pass 9 (Retirement), pass 10 (PracticeToWealth), pass 13 (StrategyComparison) | 0.8 | All 3 wealth-engine pages now read + write the shared profile. StrategyComparison late-hydrates 5 fields, drops a banner in the inputs card, persistToProfile on run, records the comparison to the run timeline with top-strategy value. |
...
| G11 | Calculator result breadcrumb: each run persists to a shared timeline across calculators | observability | P2 | done | build | pass 11 | 0.8 | New `client/src/stores/runTimeline.ts` pure helper (parseTimeline + appendEntry with 10s dedupe window + 200-entry cap + timelineStats + groupByTool + 22 tests). `useRunTimeline` hook with cross-tab sync. RunTimelinePanel modal with stats strip + entry list + Reopen/Delete/Clear actions + `stewardly-timeline-reopen` custom event for deep-link rehydration. Wired into QuickQuoteHub header as "Timeline" button with live count badge. BusinessIncomeQuickQuote + HolisticComparison auto-record runs after their mutations resolve with input/output summaries, confidence, route, and input blob. |
...
| G14 | Proactive calc suggestions based on profile deltas (life event → run Monte Carlo) | planning | P2 | done | build | pass 12 | 0.8 | New `client/src/stores/lifeEventDetector.ts` pure helper with 12-event taxonomy (marriage/divorce/new_dependent/empty_nest/business_entry/exit/hnw_crossing/income_spike+drop/retirement_approach/retirement/estate_exposure), severity tiers with stable sort, delta field for numeric events, 26 unit tests. LifeEventsBanner React component with per-event icon + tone-aware card + Open CTA + dismiss + "Mark all handled" (resets snapshot). Wired above the recommendations row in QuickQuoteHub. Snapshot cached in localStorage so events only fire on real deltas. |
