...
| PARITY-CALC-0003      | Dedicated UWE/BIE configurator UIs               | calculators    | open         | P2       | build      | 0     |             | UWE has full product-level customization (14 types) and BIE has role/team/channel modeling — both have tRPC endpoints but no dedicated configurator pages. Only accessible via Strategy Comparison / Practice to Wealth indirectly. |
| PARITY-CALC-0004      | Planning stub pages disconnected from backends   | calculators    | in_progress  | P1       | build      | 2     | (CBLP2)     | CBLP2: Tax Planning wired to tax.projectYear + projectStateTax (interactive filing/income/state inputs). Estate Planning wired to UWE estate model + estate.parseDocumentOffline (interactive estate tax calc + document parser). Income Projection wired to UWE.simulate + Monte Carlo (editable income sources + portfolio sustainability). 3 of 6 pages wired. Remaining: Risk Assessment, Insurance Analysis, Protection Score (already functional but no persistence). |
