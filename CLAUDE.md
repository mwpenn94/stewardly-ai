...
116 pages, 351 tables, 258 services, 78 routers, 17 seed files, 37 cron jobs, 129 components, 35 navigation items
Tests: 3,093 passing across 121 files in local dev (121 total with 14 pre-existing env-dependent failures that pass in the deployed environment with DB + env vars present; full suite is 3,205 tests)
Current state: ~97% deep, 3% human-dependent (env vars, GHL, compliance). 56 recursive passes converged (9.5/10; pass 45 fixed pdfReportGenerator invokeLLM bypass; pass 46 mounted 3 webhook routers; passes 47-48 confirmed clean; pass 49 wired Download Report PDF on EngineDashboard; pass 50 confirmed clean; pass 51 fixed bugfix-streaming-notification test achieving 100% pass in deployed env; pass 52 verified; pass 53 applied weight_presets migration + SCUI engine dedup + docs/ENV_SETUP.md; pass 54 closed the "reachability gap" flagged by the user — added AI Agents + Code Chat to nav, added GitHub tab to /code-chat with githubStatus + githubListOpenPRs admin procedures, implemented importEMBAFromGitHub fetcher + Import button in ContentStudio that pulls real content (366+ definitions + chapters + subsections + questions + flashcards) from mwpenn94/emba_modules, documented GITHUB_TOKEN + EMBA_IMPORT_ON_BOOT + related env vars, and added 11 regression tests (5 embaImport + 6 navReachability) that lock in the invariants; passes 55-56 confirmed 2 consecutive clean with no actionable issues). 0 TS errors, 0 TODOs.
...
- `server/routers/codeChat.ts` + `client/src/pages/CodeChat.tsx` — admin file browser + roadmap CRUD + word-diff visualizer (Round B5) + GitHub integration status/PR tab (pass 54). Routed in ADMIN_NAV (`/code-chat`, admin only). tRPC procs `codeChat.githubStatus` + `codeChat.githubListOpenPRs` gate on `adminProcedure`.
