 ...
 *
 * Wired to:
 * - workflow.listAll → real workflow checklist items
 * - compliance.getReviews → real compliance review items
 * Aggregates both into a unified work queue with status filtering.
 */
...
export default function MyWork() {
  ...
  const workflowsQ = trpc.workflow.listAll.useQuery(undefined, {
  ...
  const items = useMemo<WorkItem[]>(() => {
    ...
    // Map workflow items
    for (const wf of (workflowsQ.data ?? []) as any[]) {
      const steps = Array.isArray(wf.steps) ? wf.steps : [];
      const totalSteps = steps.length;
      const completedSteps = steps.filter((s: any) => s.completed || s.done).length;
      const allDone = totalSteps > 0 && completedSteps === totalSteps;

      result.push({
        id: `wf-${wf.id}`,
        type: "workflow",
        title: wf.workflowType?.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()) ?? "Workflow",
        subtitle: `${completedSteps}/${totalSteps} steps completed`,
        status: allDone ? "complete" : completedSteps === 0 ? "action-needed" : "in-progress",
    ...
    // Map compliance reviews
    for (const cr of (complianceQ.data ?? []) as any[]) {
      const status = cr.status === "approved" ? "complete"
        : cr.status === "flagged" ? "action-needed"
      ...
      result.push({
        id: `cr-${cr.id}`,
        type: "compliance",
        title: cr.contentType
          ? `${cr.contentType.replace(/_/g, " ")} Review`
          : "Compliance Review",
        subtitle: cr.riskLevel ? `Risk: ${cr.riskLevel}` : undefined,
        status,
        priority: cr.riskLevel === "high" ? "high" : cr.riskLevel === "medium" ? "medium" : "low",
    ...
    const statusOrder = { "action-needed": 0, "in-progress": 1, review: 2, complete: 3 };
    result.sort((a, b) => (statusOrder[a.status] ?? 4) - (statusOrder[b.status] ?? 4));

    return result;
  }, [workflowsQ.data, complianceQ.data]);

  const isLoading = workflowsQ.isLoading || complianceQ.isLoading;
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 pb-20 md:pb-6">
      ...
      <QueryErrorBanner query={complianceQ} label="compliance reviews" />
