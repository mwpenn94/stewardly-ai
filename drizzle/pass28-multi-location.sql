-- Pass 28: Multi-Location Tenant-Isolated Sync
-- New tables: ghl_locations, user_locations
-- ALTER: lead_pipeline ADD location_id, integration_webhook_events ADD location_id
...
CREATE TABLE IF NOT EXISTS `ghl_locations` (
  ...
  KEY `idx_ghl_locations_location_id` (`location_id`),
...
-- Add location_id to lead_pipeline (nullable for backward compat with existing data)
...
-- Add location_id to integration_webhook_events
...
-- Add location_id to sync_run_history
...
-- Seed the current default GHL location
